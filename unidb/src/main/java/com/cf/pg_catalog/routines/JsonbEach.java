/**
 * This class is generated by jOOQ
 */
package com.cf.pg_catalog.routines;


import com.cf.pg_catalog.PgCatalog;

import javax.annotation.Generated;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.8.3"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class JsonbEach extends AbstractRoutine<Void> {

    private static final long serialVersionUID = -249336764;

    /**
     * The parameter <code>pg_catalog.jsonb_each.from_json</code>.
     */
    public static final Parameter<Object> FROM_JSON = createParameter("from_json", org.jooq.impl.DefaultDataType.getDefaultDataType("jsonb"), false, false);

    /**
     * The parameter <code>pg_catalog.jsonb_each.key</code>.
     */
    public static final Parameter<String> KEY = createParameter("key", org.jooq.impl.SQLDataType.CLOB, false, false);

    /**
     * The parameter <code>pg_catalog.jsonb_each.value</code>.
     */
    public static final Parameter<Object> VALUE = createParameter("value", org.jooq.impl.DefaultDataType.getDefaultDataType("jsonb"), false, false);

    /**
     * Create a new routine call instance
     */
    public JsonbEach() {
        super("jsonb_each", PgCatalog.PG_CATALOG);

        addInParameter(FROM_JSON);
        addOutParameter(KEY);
        addOutParameter(VALUE);
    }

    /**
     * Set the <code>from_json</code> parameter IN value to the routine
     */
    public void setFromJson(Object value) {
        setValue(FROM_JSON, value);
    }

    /**
     * Get the <code>key</code> parameter OUT value from the routine
     */
    public String getKey() {
        return get(KEY);
    }

    /**
     * Get the <code>value</code> parameter OUT value from the routine
     */
    public Object getValue() {
        return get(VALUE);
    }
}
